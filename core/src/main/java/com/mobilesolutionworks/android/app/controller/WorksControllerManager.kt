...
//    }
     ...
     */
    fun initWorksController(id: Int, args: Bundle?, callback: (WorksControllerManager) -> WorksController): WorksController {
        val controller: WorksController? = controllers.get(id)
        if (controller == null) {
            val newController = callback(this)
    ...
    inline fun <reified D : WorksController> initController(id: Int, args: Bundle?, noinline callback: (WorksControllerManager) -> D): D {
    ...
    class ControllerManagerLoaderCallbacks(context: Context) : LoaderManager.LoaderCallbacks<WorksControllerManager> {
        ...
        override fun onCreateLoader(id: Int, args: Bundle?): Loader<WorksControllerManager> {
            return ControllerManager(context)
        }
